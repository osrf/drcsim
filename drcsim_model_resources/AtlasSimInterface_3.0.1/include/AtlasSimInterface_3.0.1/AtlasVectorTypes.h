...
public:
	...
	inline float w() const {return n[3];}   //!<  w value of position
	...
	inline float rz() const {return n[0];}  //!<  z axis rotation if vector is interpreted as euler angle set
	inline float rx() const {return n[1];}  //!<  x axis rotation of euler angle set
	inline float ry() const {return n[2];}  //!<  y axis rotation of euler angle set
	...
	inline float yaw()   const {return n[0];}  //!<  z axis rotation if vector is interpreted as euler angle set
	inline float roll()  const {return n[1];}  //!<  x axis rotation of euler angle set
	inline float pitch() const {return n[2];}  //!<  y axis rotation of euler angle set
...
public:
	...
	inline AtlasVec3f to_zxy_euler(){
		const float s = 2.0f/(m_qw*m_qw+m_qx*m_qx+m_qy*m_qy+m_qz*m_qz);

		float yaw = atan2f(-m_qx*m_qy*s+m_qw*m_qz*s,1.0f-m_qx*m_qx*s-m_qz*m_qz*s);
		float roll = atan2f(m_qy*m_qz*s+m_qw*m_qx*s,sqrtf(m_qx*m_qx*m_qz*m_qz*s*s-2.0f*m_qx*m_qz*s*s*m_qw*m_qy+
			m_qw*m_qw*m_qy*m_qy*s*s+1.0f-2.0f*m_qx*m_qx*s-2.0f*m_qy*m_qy*s+m_qx*m_qx*m_qx*m_qx*s*s+2.0f*m_qx*
			m_qx*s*s*m_qy*m_qy+m_qy*m_qy*m_qy*m_qy*s*s));
		float pitch = atan2f(-m_qx*m_qz*s+m_qw*m_qy*s,1.0f-m_qx*m_qx*s-m_qy*m_qy*s);

		return AtlasVec3f(yaw,roll,pitch);
	}
};
