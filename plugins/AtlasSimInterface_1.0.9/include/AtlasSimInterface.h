...
public:
	...
	///////////////////////////////////////////////////////
	//!
	//! \brief  Accessor to Atlas internal position estimate with out calling process_control_input
	//!
	//! \_Description
	//!
	//!    This function will return Atlas internal odometry estimates with out having to call
	//!    process_control_input.  It will also return a flag indicating if the data is valid or not.
	//!
	//! \_Parameters
	//!
	//!    \_out  robot_pos_est - Atlas internal odometry position estimate of robot base (pelvis). 
	//!    \_out  foot_pos_est  - Atlas internal odometry position estimate of feet. 
	//!
	//!    Position estimates are with respect to an integrated odometry frame which starts at (0,0) 
	//!    Robot base orientation is assumed to be consistent with current IMU orientation.
	//!
	//! \_Returns
	//!
	//!    FALSE on valid pose estimate, TRUE on invalid pose estimate
	//!
	bool get_estimated_position(AtlasPositionData & robot_pos_est, AtlasVec3f foot_pos_est[Atlas::NUM_FEET]);
