...
class AtlasTeleop():
    ...
    directions = {'u': {"forward":1, "lateral":0, "turn": 1}, \
                  ...
                  'O': {"forward":1, "lateral":0, "turn": -1}, \
                  'j': {"forward":0, "lateral":0.5, "turn": 0}, \
                  'J': {"forward":0, "lateral":0.5, "turn": 0}, \
                  'k': {"forward":0, "lateral":0, "turn": 0}, \
                  'K': {"forward":-1, "lateral":0, "turn": 0}, \
                  'l': {"forward":0, "lateral":-0.5, "turn": 0}, \
                  'L': {"forward":0, "lateral":-0.5, "turn": 0}, \
    ...
    params = {"stride_length":{ "value":0.25, "min":0, "max":1, "type":"float"},
              ...
              "sequence_length":{"value":4, "min":2, "max":100, "type":"int"},
              "stride_width":{"value":0.2, "min":0, "max":1, "type":"float"},
              "turn_size":{"value":math.pi/16, "min":0.01, "max":math.pi/2, "type":"float"},
    ...
    def twist(self, forward, lateral, turn):
        print("Walking + " + str(self.params["sequence_length"]["value"]) + " steps")
        steps = []
        
        L = self.params["stride_length"]["value"]
        y_offset = self.params["stride_width"]["value"]/2
        X = 0
        Y = 0
        theta = 0
        dTheta = 0
        ...
        if forward != 0:
            dTheta = turn * 2 * math.asin(L/(2*self.params["turn_radius"]["value"]))
        else:
            dTheta = turn * self.params["turn_size"]["value"]
        steps = []
        ...
        for i in range(self.params["sequence_length"]["value"]):
            ...
            dX = forward - foot * turn * math.sin(dTheta)
            dY = lateral - foot * turn * math.cos(dTheta)
            
            X += dX * self.params["stride_length"]["value"]
            Y += dY * self.params["stride_length"]["value"]
            print("forward " + str(forward) + " lateral " + str(lateral) + " turn " + str(turn))
            print("X " + str(X) + " Y " + str(Y) + " theta " + str(theta))
            #X += dX / math.sqrt(dX*dX + dY*dY) + math.sin(theta) * (i%2)*0.2
            #Y += dY / math.sqrt(dX*dX + dY*dY) + math.cos(theta) * (i%2)*0.2
